...
4. Code
  ...
  res.debug_2 =
    'After successfully executing the pnpm dm compilation, you can either mock the request to the locally compiled mock.json, or input the development machine IP address, preview it on the mobile device, or Hot Restart it. For details, please refer to the octoflutter-android/octoflutter-ios project.'
  res.debug_3 =
    'For debug JS On the Android device, you should connect to USB, run octoflutter-android project, pay attentation to the log tag [AppBundleDevSupplier], check the port number output from this tag, cd to ./dev in octoflutter-android project, execute [sh octo_flutter_dev. sh $port] after launch your appbundle, now, you can debug js through the Chrome Dev Tools. \n\nFor iOS, you can debug js through attach JSContext from Safari.'
